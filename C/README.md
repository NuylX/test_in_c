# 🧱 Estudo da Linguagem C

![Status](https://img.shields.io/badge/Nível-Em%20Estudo-blue)
![Foco](https://img.shields.io/badge/Foco-Estruturas%20básicas%20|%20Condicionais%20|%20Switch%20Cases-green)

---

## 📖 Em estudo atualmente

- Introdução à estrutura básica da linguagem C  
- Uso de condicionais (`if`, `else`, `else if`)  
- Estrutura de decisão com `switch case` para menus e escolhas do usuário  
- Compilação e execução via terminal (Windows / Linux)  

---

## 📂 Estrutura do Diretório

`basico/` – exemplos práticos em C:
- [calcular_media.c](basico/calcular_media.c)
- [desafios.c](basico/desafios.c)
- [dados_pessoais.c](basico/dados_pessoais.c)
- [fliperama.c](basico/fliperama.c)
- [soma.c](basico/soma.c)
- [potencias.c](basico/potencias.c)

---

## ⚙️ Como Compilar e Executar

### 🪟 Windows (PowerShell)
```powershell
gcc basico/dados_pessoais.c -o basico/dados_pessoais.exe
.\basico\dados_pessoais.exe
```

### 🐧 Linux / macOS
```
gcc basico/dados_pessoais.c -o basico/dados_pessoais
./basico/dados_pessoais
```

---

[⬅️ Voltar ao principal](../README.md)
